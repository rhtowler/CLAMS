...
class seabirdEvent(QDialog, ui_ABLTrawlEvent.Ui_ABLTrawlEvent):
    ...
    def __init__(self, parent=None):
        ...
        super(seabirdEvent, self).__init__(parent)
        self.setupUi(self)
        ...
        self.depthStartBtn.setVisible(False)
    ...
    def initTrawlEventDialog(self):
        ...
        while query.next():
            self.gear.append(query.value(0).toString())
        if 'Seabird Survey' in self.gear:
            self.gearBox.addItem('Seabird Survey')
    ...
    def getEventData(self):
        ...
        if self.buttons[ind].text().endsWith('TimeStamp'):
            
            partition='MainTrawl'
            parameter=['TimeStamp', 'EQLatitude', 'EQLongitude']
            if not self.reloaded:
                self.fishingFlag=False
                self.SCSLogInterval = self.streamEQHBLogInterval
    ...
    def exitCheck(self):
        ...
        if (self.fishingFlag and (self.gear <> 'CTD')):
            self.message.setMessage(self.errorIcons[0],self.errorSounds[0],"Please press TimeStamp button!", 'info')
            self.message.exec_()
