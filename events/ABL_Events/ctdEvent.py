...
class ctdEvent(QDialog, ui_ABLTrawlEvent.Ui_ABLTrawlEvent):
    ...
    def __init__(self, parent=None):
        ...
        super(ctdEvent, self).__init__(parent)
        self.setupUi(self)
    ...
    def initTrawlEventDialog(self):
        ...
        query = QtSql.QSqlQuery("SELECT gear FROM " + self.schema + ".gear WHERE active=1 " +
                " AND gear_type = 'CTD_Station' ORDER BY gear_gui_order", self.db)
        while query.next():
    ...
    def getEventData(self):
        ...
        if self.reloaded:
            ...
            if len(parameter)>1 and self.gear <> 'CTD' and self.gear <> 'TowFish':
    ...
    def exitCheck(self):
        ...
        if (self.depthEndBtn.text() == '' and self.gear == 'CTD'):
            self.message.setMessage(self.errorIcons[0],self.errorSounds[0],"Select a max depth!",'info')
            self.message.exec_()
            self.incomplete=True
            return

        if (self.stnBtn.text() == '' and (self.gear <> 'TowFish')):
