...
# THIS CODE HAS BEEN ADAPTED BY MATT WILSON FOR USE DURING FOCI AGE-0 GROUNDFISH SURVEYS IN THE WESTERN GOA:
# 1) ADDED DOORSIN LAT & LONG
# 2) REMOVED START DEPTH AND END DEPTH
# 3) ADDED BOTTOM DEPTH FOR OUTPUT TO EVENT_DATA AT DOORS OUT, EQ, HAULBACK, AND DOORS IN
# 4) CHANGED INITIAL BUTTON COLOR TO RED FOR DOORS OUT AND DOORS IN
# ALL CHANGES MATT MADE ARE PRECEDED BY COMMENTS BEGINNING WITH 'MATT' AND WRITTEN IN ALL CAPS
...
class FOCITrawlEvent(QDialog, ui_ABLTrawlEvent.Ui_ABLTrawlEvent):
    ...
    def __init__(self, parent=None):
        ...
        super(FOCITrawlEvent, self).__init__(parent)
        self.setupUi(self)
...
#MATT COMMENTED OUT THE FOLLOWING 2 LINES
...
##        self.connect(self.depthEndBtn, SIGNAL("clicked()"),self.getDepthEnd)
    ...
    def reloadData(self):
        ...
        #get station
        query=QtSql.QSqlQuery("SELECT parameter_value FROM " + self.schema + ".event_data WHERE ship="+self.ship+
            " AND survey="+self.survey+" AND event_id="+self.activeEvent+" AND haul_parameter='Station'")
        if query.first():
            self.stnBtn.setText(query.value(0).toString())

#MATT COMMENTED OUT THE FOLLOWING LINES
##        #get starting depth
##        query=QtSql.QSqlQuery("SELECT parameter_value FROM " + self.schema + ".event_data WHERE ship="+self.ship+
##            " AND survey="+self.survey+" AND event_id="+self.activeEvent+" AND haul_parameter='BottomDepthStart'")
##        if query.first():
##            self.depthStartBtn.setText(query.value(0).toString())
##
##        #get ending depth
##        query=QtSql.QSqlQuery("SELECT parameter_value FROM " + self.schema + ".event_data WHERE ship="+self.ship+
##            " AND survey="+self.survey+" AND event_id="+self.activeEvent+" AND haul_parameter='BottomDepthEnd'")
##        if query.first():
##            self.depthEndBtn.setText(query.value(0).toString())
        ...
        for btn in self.buttons:
            if btn.text().startsWith('C'):
                ...
                parameter=[g.split(' ')[1]]
#MATT REPLACED THE FOLLOWING LINE WITH THE NEXT ONE
##            elif btn.text() in ['EQ', 'Haulback']:
            elif btn.text() in ['EQ', 'Haulback', 'DoorsOut', 'DoorsIn']:
                partition='Codend'
                parameter=[btn.text()]
            else:
                partition='MainTrawl'
...
#MATT CHANGED THE FOLLOWING LINE TO THE NEXT ONE
##            if query.value(0).toString() in ['EQ','Haulback']:
            if query.value(0).toString() in ['EQ','Haulback', 'DoorsOut', 'DoorsIn']:
...
#MATT INSERTED THE FOLLOWING LINE
        botmz = self.dispVector[2]
        ...
        if self.buttons[ind].text().endsWith('EQ'):
            ...
            if self.gear in ['MOCC','Tucker']:
                ...
                partition='Codend'
#MATT REPLACED THE FOLLOWING LINE WITH THE ONE AFTER IT TO ADD BOTTOM DEPTH AT EQ
##            parameter=['EQ', 'EQLatitude', 'EQLongitude']
            parameter=['EQ', 'EQLatitude', 'EQLongitude', 'EQBottomDepth']
            if not self.reloaded:
                self.fishingFlag=True
                self.SCSLogInterval = self.streamEQHBLogInterval
            ...
            if self.gear in ['MOCC','Tucker']:
                ...
                partition='Codend'
#MATT REPLACED THE FOLLOWING LINE WITH THE ONE AFTER IT TO ADD BOTTOM DEPTH AT HAULBACK
##            parameter=['Haulback', 'HBLatitude', 'HBLongitude']
                parameter=['Haulback', 'HBLatitude', 'HBLongitude', 'HBBottomDepth']
            if not self.reloaded:
                self.fishingFlag=False
                self.SCSLogInterval = self.streamSlowLogInterval
                self.netdlg.setTime=time
...
##            parameter=['DoorsOut', 'DOLatitude', 'DOLongitude']
            parameter=['DoorsOut', 'DOLatitude', 'DOLongitude', 'DOBottomDepth']
            if not self.reloaded:
                self.fishingFlag=False
                self.SCSLogInterval = self.streamSlowLogInterval

#MATT INSERTED THE FOLLOWING LINES
        elif self.buttons[ind].text().endsWith('DoorsIn',Qt.CaseInsensitive):
            partition='Codend'
            parameter=['DoorsIn', 'DILatitude', 'DILongitude', 'DIBottomDepth']
            if not self.reloaded:
                self.fishingFlag=False
                self.SCSLogInterval = self.streamSlowLogInterval
#MATT END OF INSERTION
        ...
        if self.reloaded:
            ...
            if self.timeDlg.exec_():
                ...
                lat = self.dispVector[0]
                lon = self.dispVector[1]
#MATT INSERT THE FOLLOWING LINE TO EXTRACT BOTTOM DEPTH
                botmz = self.dispVector[2]
                ...
                if not gotVal:
                    ...
                    if len(parameter)>1:
                        ...
                        self.backLogger.info(QDateTime.currentDateTime().toString('MMddyyyy hh:mm:ss')+","+query.lastQuery())
#MATT INSERT LINES SO THAT BOTTOM DEPTH IS INSERTED WITH EQ OR HAULBACK TOO
                        query=QtSql.QSqlQuery("INSERT INTO " + self.schema + ".event_data (ship, survey, event_id, " +
                                "partition, event_parameter, parameter_value) VALUES ("+ self.ship+","+
                                self.survey+","+self.activeEvent+",'"+partition+"','"+parameter[3]+"','" + botmz+"')")
                        self.backLogger.info(QDateTime.currentDateTime().toString('MMddyyyy hh:mm:ss')+","+query.lastQuery())
#MATT END INSERTION
                    ...
                    if len(parameter)>1:
                        ...
                        self.backLogger.info(QDateTime.currentDateTime().toString('MMddyyyy hh:mm:ss')+","+query.lastQuery())
#MATT INSERT LINES SO THAT BOTTOM DEPTH IS INSERTED WITH EQ OR HAULBACK TOO
                        query=QtSql.QSqlQuery("UPDATE " + self.schema + ".event_data SET parameter_value='"+botmz+
                                "' WHERE ship="+self.ship+" AND survey="+self.survey+" AND event_id="+
                                self.activeEvent+" AND partition='"+partition+ "' AND event_parameter='"+
                                parameter[3]+"'")
                        self.backLogger.info(QDateTime.currentDateTime().toString('MMddyyyy hh:mm:ss')+","+query.lastQuery())
...
#MATT INSERT LINES SO THAT BOTTOM DEPTH IS INSERTED WITH EQ OR HAULBACK TOO
                    query = QtSql.QSqlQuery("INSERT INTO " + self.schema + ".event_data (ship, survey, event_id, " +
                            "partition, event_parameter, parameter_value) VALUES (" + self.ship + ","+self.survey+","+
                            self.activeEvent + ",'"+partition + "','" + parameter[3] + "','" + botmz + "')")
                    self.backLogger.info(QDateTime.currentDateTime().toString('MMddyyyy hh:mm:ss')+","+query.lastQuery())
#MATT END INSERTION
